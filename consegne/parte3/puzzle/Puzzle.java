 ...
 * @author svalle
 */
public class Puzzle extends UnicastRemoteObject implements GruppoOrdinabile {
	private int rows;
	private int cols;
	 ...
	 * @param rows	array contenente le righe dalle quali leggere i tasselli
	 */
	@Override
		...
		public void run() {
				if(top.equals("n")){
					...
					for(int j = 1; j < rows; j++) {
						...
						PuzzleItem current = null;
						try {
							current = mucchio.getPiece(prevId);
						} catch (RemoteException re) {}
	...
	/**
	 * <p>Metodo che restituisce la matrice ordinata che rappresenta il puzzle da scrivere su un file di output .</p>
	 */
	@Override
	...
	public synchronized boolean isEmpty() {
		...
		try { if(!mucchio.isEmpty()) somethingPresent = true; } catch(RemoteException re) {}
		for(int i = 0; i < rows; i++)
			for(int j = 0; j < cols; j++)
				if(puzzle[i][j] != null)
	...
	public synchronized PuzzleItem getPiece(String id) {
		PuzzleItem piece = null;
		try {
			piece = mucchio.getPiece(id);
		} catch (RemoteException re) {}
	...
	public synchronized PuzzleItem getEdgePiece(Dir edge, Dir side, String ref) {
		PuzzleItem item = null;
		try {
			item = mucchio.getEdgePiece(edge, side, ref);
		} catch (RemoteException re) {}
		if(item != null)
			return item;
		int line = 0;
